...
--
-- Host: 127.0.0.1
-- Generation Time: Jan 24, 2025 at 03:38 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SuperAdmin_AddMunicipality` (IN `region_id_val` INT, IN `scc_id_val` INT, IN `province_or_city_id_val` INT, IN `municipality_name_val` VARCHAR(255), IN `municipality_head_val` VARCHAR(255), IN `created_by_val` VARCHAR(255))   BEGIN

    INSERT INTO municipality (region_id, cluster_id, province_or_city_id, municipality_name, municipality_head, created_by, updated_by, created_on, updated_on, status)
    SELECT 
        region_id_val, 
        scc_id_val, 
        province_or_city_id_val, 
        municipality_name_val, 
        municipality_head_val, 
        created_by_val, 
        created_by_val, 
        CURRENT_TIMESTAMP, 
        CURRENT_TIMESTAMP, 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SuperAdmin_GetAllMunicipality` (IN `pageNo` INT, IN `pageSize` INT, IN `keyword` VARCHAR(255))   BEGIN
    ...
    WHERE  (
        ...
        OR municipality_head LIKE CONCAT('%', keyword, '%')
    )
...
(3, 1, 2, 'section', 'Region 10', ' ', 'super_admin', 'super_admin', '2025-01-06 08:40:59', '2025-01-06 08:40:59', 'active');
