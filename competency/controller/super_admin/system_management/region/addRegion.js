...
const json_exist = {"result":"EXIST","message":"Region already exist or Region Admin does not exist"};
...
const handler = (event, callback) => {
    debug && console.log("Add Region Handler")

    const { regionName, regionAdmin, username } = event.body

    const query = `call SuperAdmin_AddRegion('${regionName}', '${regionAdmin}', '${username}')`
    try {
