...
const handler = (event, callback) => {
    debug && console.log("Update Division Handler")

    const { divisionName, divisionChief, username, regionID} = event.body
    const divisionID = event.params.divisionID

    const query = `call SuperAdmin_UpdateDivision(${divisionID}, ${regionID}, '${divisionName}', '${divisionChief}', '${username}')`
    try {
