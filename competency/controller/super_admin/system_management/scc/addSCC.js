...
const json_exist = {"result":"SUCCESS","message":"Section, Cluster, City already exist or Cluster Head does not exist"};
...
const handler = (event, callback) => {
    debug && console.log("Add SCC Handler")

    const { sccName, clusterHead, type, provinceID, regionID, username } = event.body

    const query = `call SuperAdmin_AddSCC('${sccName}', ${regionID},  ${provinceID}, '${type}', '${clusterHead}', '${username}')`
    try {
