...
const json_exist = {"result":"SUCCESS","message":"The municipality does not exist"};
...
const handler = (event, callback) => {
    debug && console.log("Update Municipality Handler")

    const { regionID, clusterID, provinceOrCityID, municipalityName, username} = event.body

    const municipalityID = event.params.id

    const query = `call SuperAdmin_UpdateMunicipality(${municipalityID}, ${regionID},  ${clusterID}, ${provinceOrCityID}, '${municipalityName}', '${username}')`
    try {
