...
const json_exist = {"result":"SUCCESS","message":"Position already exist"};
...
const handler = (event, callback) => {
    debug && console.log("Add Position Handler")

    const { position_name } = event.body
    const created_by = event.user.username

    const query = `call SuperAdmin_AddPosition('${position_name}', '${created_by}')`
    try {
