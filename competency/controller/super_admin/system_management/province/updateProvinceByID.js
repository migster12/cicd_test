...
const json_exist = {"result":"SUCCESS","message":"The province does not exist"};
...
const handler = (event, callback) => {
    debug && console.log("Update Province Handler")

    const { provinceName, provinceDirector,regionID, username } = event.body
    const ID = event.params.id

    const query = `call SuperAdmin_UpdateProvince(${ID}, ${regionID}, '${provinceName}', '${provinceDirector}', '${username}')`
    try {
